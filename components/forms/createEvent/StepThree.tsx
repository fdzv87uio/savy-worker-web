...
const uploadFileHandler = async (file: File, token: string) => {
    try {
...
export default function StepThree() {
    ...
    const onSubmit: SubmitHandler<Inputs> = async (data) => {
        ...
        if (images.length > 0) {
            fileUploadResponse = await uploadFileHandler(images[0].file, token);
        } else if (uploadedFiles.length > 0) {
            fileUploadResponse = await uploadFileHandler(uploadedFiles[0], token);
        }
    ...
    return (
        <form className={`absolute w-[250px] md:w-[450px] mt-12 flex flex-col gap-4`} onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex flex-col items-left gap-[5px]">
                <p className={`pl-2 text-[#ffffff] font-normal ${inter.className} text-sm`}>Upload images</p>
                <ImageUploading
                    multiple
                    value={images}
                    onChange={onChange}
            ...
            {isUploading ? (
                ...
                <Button
                    ...
                    variant="primary"
                    size="sm"
                    className={`w-[95px] h-[36px] px-4 py-2 text-sm font-normal ${inter.className} mt-3`}
