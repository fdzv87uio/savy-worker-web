        ...
        <div className="w-full h-[70px] mt-[60px] mx-[5px] my-[5px] flex flex-col justify-center items-center">
            <div className="w-auto h-[70px] flex flex-row justify-between">
                ...
                {array.map((x: any, key: number) => {
                    return (
                        <div key={`button_${key}`} className={`${currentPage === x ? 'bg-primary-1/50' : ''} w-[50px] h-[50px] flex flex-col justify-center items-center cursor-pointer rounded-full`}>
                            <div onClick={() => { handlePageChange(x) }}
                                className="flex flex-col justify-center items-center hover:bg-white-400/50">
