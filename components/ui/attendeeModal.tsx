                ...
                <div className="w-full h-auto flex flex-col items-left">
                                    ...
                                    <div className={`cursor-pointer py-2 flex flex-col justify-center items-left p-1 w-[95%] m-1 min-h-[60px] h-auto hover:outline hover:outline-offset-2 hover:outline-2 hover:outline-primary bg-transparent border-spacing-[2px] border-[2px] border-[#C4C4C4] rounded-xl ${inter.className} font-normal text-[#000000]`}>
                                        <div className="flex flex-row items-center px-2.5 gap-2">
                                            ...
                                            <div className="flex flex-col md:flex-row items-left mr-4 md:mr-0 md:items-center items-left gap-1 md:gap-2">
                                                ...
                                                <span className=" text-xs text-left"><strong>{detail}</strong></span>
                                                <span className=" hidden md:flex text-base text-left"><strong>-</strong></span>
                                                <span className=" text-xs text-left"><strong>{x.roles[0]}</strong></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className={`w-full h-[170px] mt-4 flex flex-col overflow-y-scroll ${selection?.length === 0 ? "justify-center items-center" : ""} flex-col gap-1`}>
                        {selection && selection.length > 0 && selection.map((x: any, key: number) => {
                            return (
                                <div key={`attendee_${key}`} className="px-2 w-full flex flex-row h-[37px] items-center justify-between bg-gray-500/50 hover:bg-gray-500 text-[#ffffff] border border-[#ffffff] rounded-lg">
                                    <span>{x}</span>
